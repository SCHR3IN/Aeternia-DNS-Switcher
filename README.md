...
<p align="center">
  <img src="https://img.shields.io/badge/version-2.4.4-blue" alt="Version">
...
**Установка или восстановление старой/сломанной установки — одной командой.**
...
недостающие зависимости, заменит старый Linux-лаунчер и откроет приложение.
Если Homebrew отсутствует, запускается его [официальный установщик](https://brew.sh).
...
Версия 2.4.4 использует отдельный системный помощник. Пароль администратора
...
На macOS повторите команду быстрой установки выше или запустите новый архив через `sudo bash install.sh`.
Пароль разрешает обновление самого системного помощника; обычное переключение его не требует.

На Linux также можно обновить через клавишу `U` в интерфейсе программы.
...
| `M` | macOS: сменить режим выбранной страны — DNS ⇄ NAVIS (WARP) |
| `W` | macOS: настроить профиль WARP (импорт из ATLAS или регистрация у Cloudflare) |
