...
class WarpProfileTests(unittest.TestCase):
    ...
    def test_navis_config_targets_country_through_warp(self):
        cfg = h.navis_config({'code': 'nl', 'user_id': '0f1e2d3c'}, PROFILE, ['1.2.3.4'])
        doh = cfg['dns']['servers'][0]
        self.assertEqual((doh['server'], doh['path'], doh['detour']),
                         ('nl.aeternia.space', '/dns-query/0f1e2d3c', 'warp'))
        ...
        plain = h.navis_config({'code': 'nl', 'user_id': '0f1e2d3c'}, PROFILE, [], obfuscated=False)
